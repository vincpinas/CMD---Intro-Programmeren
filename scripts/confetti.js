let maxParticleCount = 170; //set max confetti count
let particleSpeed = 0.02; //set the particle animation speed
...
const confetti = function() {
	...
	function startConfettiInner(canvasSelector) {
	...
	function drawParticles(context) {
		...
		for (let i = 0; i < particles.length; i++) {
			let modifier = Math.random() * 4 + 2;
			particle = particles[i];
			context.beginPath();
			context.lineWidth = particle.diameter;
			context.strokeStyle = particle.color;
			x = particle.x + particle.tilt;
			context.moveTo(x + particle.diameter / modifier, particle.y);
			context.lineTo(x, particle.y + particle.tilt + particle.diameter / modifier);
			context.stroke();
		}
	}
